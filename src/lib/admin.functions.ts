...
export const registrarMovimento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const db = await admin(context);
    const { error } = await db.rpc("admin_registrar_movimento", { p_slug: data.slug, p_quantidade: data.quantidade, p_tipo: data.tipo, p_motivo: data.motivo?.trim() || undefined, p_user_id: context.userId, p_cliente_id: data.cliente_id || undefined });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
