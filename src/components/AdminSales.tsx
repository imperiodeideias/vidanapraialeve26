...
export function AdminSales() {
  ...
  return <section className="mt-6 space-y-6" aria-label="Relatório de vendas">
    ...
    {valido && data && <>
      <div className="grid gap-4 sm:grid-cols-3">{[["Total de vendas",data.totalVendas],["Valor total de vendas",money(data.valor)],["Unidades vendidas",data.unidades]].map(([label,value])=><div key={label} className="rounded-2xl border bg-card p-5"><p className="text-sm">{label}</p><p className="text-3xl mt-3">{value}</p></div>)}</div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border bg-card p-5 min-w-0"><h3 className="text-lg mb-4">Valor vendido por dia</h3><div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.dias}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="dia" tickFormatter={v=>v.slice(8)+"/"+v.slice(5,7)}/><YAxis tickFormatter={v=>money(v)}/><Tooltip formatter={(v:number)=>money(v)}/><Bar name="Vendas" dataKey="valor" fill="#1F5D63"/></BarChart></ResponsiveContainer></div></section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section><h3 className="text-xl mb-4">Produtos mais vendidos</h3><ol className="space-y-2">{data.maisVendidos.map((p,i)=><li key={p.slug} className="rounded-xl border p-4 flex justify-between gap-4"><span>{i+1}. {p.nome}</span><strong className="shrink-0">{p.quantidade} un.</strong></li>)}</ol></section>
